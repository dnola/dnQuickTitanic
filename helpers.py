...
class PandasEncoder:                # All we need to put something in a pipeline is a class with fit(self,X,y=None) and transform(self,X)
    def fit(self,X,y=None):
        ...
        self.columns_in_train_data = X.columns
        return self                 # fit() returns self

    def transform(self,X): # transform() returns the transformed data
        return pd.get_dummies(X).reindex(columns = self.columns_in_train_data, fill_value=0) # Sometimes test data and train data have different columns in them after one hot encoding - so we throw away anything that we havent seen in train data
